...
// ref: https://www.cnblogs.com/huxiao-tee/p/4660352.html
...
handle_sigsegv(int sig, siginfo_t *si, void *ctx)
{
  ...
  fault_addr = align_down(fault_addr, page_size);
  if (last && munmap(last, page_size) == -1) {
    fprintf(stderr, "Couldn't munmap() region for sqrt; %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
  last = mmap(fault_addr, page_size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
  if (last == MAP_FAILED) {
    fprintf(stderr, "Couldn't mmap() region for sqrt; %s\n", strerror(errno));
    exit(EXIT_FAILURE);
  }
  calculate_sqrts(last, (last-sqrts), page_size/sizeof(double));
}
